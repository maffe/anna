"""functions that allow manipulation of the database of unique user identities.
these functions include things like fetching the name corresponding to a certain
id or vice-versa, adding new identities, etc."""
...
#TODO: this module too, needs a lot of cleaning.

import mysql
import stringfilters as filters

def addUid(name,typ):
	"""add a new uid to the database. the typ argument is passed directly to the
	function for checking types, the name is inserted in the database. Do note
	that if self.getTypeId() raises an exception, it will not be catched. If the
	uid already exists it will return its uid.

	fixme; this function is pretty ugly. addUid() shouldn't return the uid of an
	already existing entry. also, it shouldn't use try: except: statements if it's
	going to check for it anyway. (only use try: except: if you don't expect
	something to happen!)"""
	try:
...
def getId(name, typ):
	...
	try:
		n = cursor.execute("select `id` from `uids` where `name`=%s and `type`=%s limit 1", (name, typ))
		#.execute() returns the number of rows
	except cursor.ProgrammingError, e:
		print "\n\n  >> DATABASE ERROR: QUERY FAILED <<\n\n  >> %s\n\n" % e
		cursor.close()
...
def getType(typeid):
	"""returns the conversation type with the specified id (int||str). note that
	int(id) is executed without for an exception."""
	query = "select `type` from `convotypes` where `id` = %s limit 1;"
	cursor = mysql.db_r.cursor()
	n = cursor.execute(query, typeid)
	if not n:
		cursor.close()
...
def getTypeId(typ):
	"""returns the id (int) of the supplied typ(str), fetched from the database
	that holds the different conversation types. the ValueError exception is raised
	if it is not an existant conversation type."""
