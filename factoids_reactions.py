...
Module that takes care of everything you need to do with factoids and
reactions:
add()
delete()
exists()
...
- 3: factoid is protected"""
	...
	def exists(self,object):
		'''return True (bool) if an entry exists for object (unicode) in the factoids
table, False if not. return 0 (int) if the query failed. it returns 0 for all
errors instead of a specific integer for each seperate error because this allows
one to do "if exists('bla'):" without getting a false "true" when an error occurs.'''
...
2: db error: query failed"""
		...
		try:
			cursor.execute(
				"insert into `" + self.table_name + "`(`message_in`,`message_out`,`uid`) values(%s,%s,%s);",
...
2: database error: query failed"""
		...
		try:
			cursor.execute(
				"select `message_out`,`count`,`id` from `" + self.table_name + "` where" \
				+ " `message_in` = %s limit 1;",(message_in,))
		except cursor.ProgrammingError, e:
			print "\n\n  >> DATABASE ERROR: QUERY FAILED <<\n\n  >> %s\n\n" % e.__str__()
			return 2
		result = cursor.fetchone()
...
class ReactionsGlobal( Reactions ):
	'''global reactions (contrary of the ReactionsDirect class). for example:
<user> /me dances *
<anna> /me dances too *
